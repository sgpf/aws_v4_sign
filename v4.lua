...
local function prepare_awsv4_request(tbl)
  ...
  local unsigned_payload = false
  if req_headers and req_headers["X-Amz-Content-Sha256"] == "UNSIGNED-PAYLOAD" then
  ...
  if unsigned_payload then
    hash_req_payload = req_headers["X-Amz-Content-Sha256"]
  else
    hash_req_payload = hex_encode(hash(req_payload or ""))
  end
